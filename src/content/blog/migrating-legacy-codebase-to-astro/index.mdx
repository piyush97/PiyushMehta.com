...
image:
  url: '/blog/migrating-legacy-codebase-to-astro/images/astro-migration-hero.svg'
  alt: 'Astro migration illustration showing old codebase transforming to modern Astro architecture'
draft: false
---
...
const enhanceComponent = (element: HTMLElement, enhancement: ComponentEnhancement) => {
...
const scoreFramework = (framework: Framework) => {
  return Object.entries(evaluationCriteria).reduce((score, [criterion, weight]) => {
