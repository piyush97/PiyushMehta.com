...
<div className="bg-gradient-card border border-card-border p-6 rounded-lg my-8 shadow-card">
  <h3 className="text-xl mb-2 text-text-primary">
    Google's senior SWE loop has a ~0.2% offer rate from initial application
  </h3>
  <p className="text-text-secondary">
    But candidates who prepare system design in depth instead of just memorizing Grokking pass at
    nearly 5x the average rate. The gap is depth, not breadth.
  </p>
</div>
...
<div className="border border-accent/40 my-6 bg-light-700 p-4 rounded-lg">
  ...
  <ul className="mt-2 space-y-1 text-text-secondary">
    ...
    <li>
      Mergeable: Combine two HLL counters by taking the max of each register. Perfect for
      distributed systems
    </li>
    <li>
      About 2% error: Not exact, but for "how many unique users visited today?" 2% is perfectly fine
    </li>
  </ul>
</div>
...
<div className="bg-light-700 border border-card-border p-4 rounded-lg my-6">
  ...
  <ul className="space-y-1 text-text-secondary">
    ...
    <li>
      Merge cost: HashSets require full data transfer; HLL merges in microseconds by comparing
      registers
    </li>
  </ul>
</div>
...
<div className="bg-accent/20 border border-accent/30 p-4 rounded-lg my-6">
  ...
  <p className="text-text-secondary mt-2">
    When the interviewer asks "How would you count unique users at scale?", lead with HyperLogLog.
    Mention the 1.5 KB memory footprint, the mergeability property for distributed systems, and the
    ~2% error trade-off. Then ask: "Would 2% error be acceptable for this use case, or do we need
    exact counts?" That follow-up question alone signals senior-level thinking.
  </p>
</div>
