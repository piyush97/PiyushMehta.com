...
const companies: CompanyExample[] = [
  ...
  },
  {
    ...
    details:
      'Instantly identifies known spam patterns and malicious content before expensive ML model inference.',
  ...
  },
  {
    ...
    details:
      'Lightweight clients use Bloom filters to identify relevant transactions without downloading entire blockchain.',
  ...
  },
  {
    ...
    details:
      "Local Bloom filter checks URLs against known malicious sites before querying Google's servers.",
    ...
    <div className="bg-gradient-card border border-card-border p-6 rounded-lg my-8 shadow-card">
      ...
      <div className="bg-light-800 rounded-lg p-6 shadow-card border border-card-border">
        <div className="flex items-center mb-4">
          ...
          <div>
            <h4 className="text-2xl font-bold text-text-primary">{selectedCompany.name}</h4>
            <p className="text-accent font-semibold">{selectedCompany.use_case}</p>
          </div>
        </div>

        <div className="grid md:grid-cols-2 gap-6">
          <div>
            <h5 className="font-semibold text-text-primary mb-2">How They Use It:</h5>
            <p className="text-text-secondary leading-relaxed">{selectedCompany.details}</p>
          </div>

          <div className="space-y-4">
            <div className="bg-accent/20 border border-accent/30 p-4 rounded-lg">
              <h6 className="font-semibold text-accent mb-1">📈 Business Impact</h6>
              <p className="text-text-secondary text-sm">{selectedCompany.impact}</p>
            </div>

            <div className="bg-light-700 border border-card-border p-4 rounded-lg">
              <h6 className="font-semibold text-text-primary mb-1">⚡ Performance Stats</h6>
              <p className="text-accent text-sm font-mono">{selectedCompany.stats}</p>
            </div>
          </div>
        </div>
      </div>
