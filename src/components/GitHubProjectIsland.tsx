...
function getDefaultImageUrl(project: Project) {
...
function ProjectListItem({
  ...
  return (
    <div className="flex items-center p-6 transition-all duration-200 border rounded-lg bg-surface-secondary border-card-border hover:border-accent/50">
      <div className="flex-shrink-0 size-20 mr-6 overflow-hidden rounded-lg">
        <img
          src={project.imageUrl || getDefaultImageUrl(project)}
          alt={project.title}
...
type FilterAction =
  ...
  | { type: 'SET_TECH'; value: string }
  | { type: 'SET_FEATURED_ONLY'; value: boolean }
  | { type: 'SET_SORT'; value: 'year' | 'title' }
  | { type: 'SET_VIEW_MODE'; value: 'grid' | 'list' }
  | { type: 'CLEAR' };
...
function filterReducer(state: FilterState, action: FilterAction): FilterState {
  switch (action.type) {
    ...
    case 'SET_TECH':
      return { ...state, selectedTech: action.value };
    case 'SET_FEATURED_ONLY':
      return { ...state, showFeaturedOnly: action.value };
    case 'SET_SORT':
...
export default function GitHubProjectIsland({
  ...
  const { selectedCategory, selectedTech, showFeaturedOnly, sortBy, viewMode } = filters;
  ...
  const technologies = useMemo(() => {
    const techs = projects.flatMap((p) =>
      (p.technologies || []).flatMap((tech) => {
        const trimmed = tech?.toString().trim();
  ...
  return (
    <div className={className}>
      <div className="mb-10 space-y-6">
        <div className="flex flex-col gap-y-4 lg:gap-y-0 lg:flex-row lg:justify-between">
          <div>
          ...
          <div>
            ...
            <div className="relative">
              <select
                id="technology"
                value={selectedTech}
                onChange={(e) => dispatch({ type: 'SET_TECH', value: e.target.value })}
                className="w-full px-4 py-2 pr-10 transition-all duration-200 border rounded-lg appearance-none cursor-pointer bg-surface-primary border-card-border text-text-primary focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent hover:border-accent/50"
              >
          ...
          <div>
            ...
            <div className="relative">
              <select
                ...
                value={sortBy}
                onChange={(e) =>
                  dispatch({ type: 'SET_SORT', value: e.target.value as 'year' | 'title' })
                }
                className="w-full px-4 py-2 pr-10 transition-all duration-200 border rounded-lg appearance-none cursor-pointer bg-surface-primary border-card-border text-text-primary focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent hover:border-accent/50"
              >
          ...
          <div className="space-y-4">
            <div className="flex items-center">
              <input
                ...
                onChange={(e) => dispatch({ type: 'SET_FEATURED_ONLY', value: e.target.checked })}
                className="size-4 transition-colors duration-200 rounded text-accent focus:ring-accent focus:ring-opacity-50"
              />
              <label
            ...
            <div className="flex items-center gap-x-2">
              ...
              <div className="flex overflow-hidden border rounded-lg border-card-border">
                ...
                <button
                  type="button"
                  onClick={() => dispatch({ type: 'SET_VIEW_MODE', value: 'list' })}
                  className={`px-3 py-1 ${
      ...
      >
        {filteredProjects.length === 0 ? (
          <div className="py-10 text-center col-span-full">
            <svg
              className="size-12 mx-auto text-text-secondary"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
            ...
            {filteredProjects.map((project) =>
              viewMode === 'grid' ? (
                <ProjectCard
                  key={project.id}
                  project={project}
                  onTechClick={(v) => dispatch({ type: 'SET_TECH', value: v })}
                />
              ) : (
                <ProjectListItem
                  key={project.id}
