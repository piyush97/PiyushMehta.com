...
export const OG_TEST_SUITE: TestConfig[] = [
  // Template Tests
  {
    ...
    description: 'Testing the modern blog template with comprehensive content and multiple tags for validation.',
  ...
  // Edge Case Tests
  {
    title: 'Very Long Title That Should Be Properly Truncated When It Exceeds The Maximum Length Limit For Social Media Platforms And URL Parameters',
    description: 'Testing extremely long titles to ensure proper truncation and handling of edge cases in URL generation and image rendering without breaking the layout or causing visual issues.',
    expectedTemplate: 'modern',
    expectedTheme: 'dark',
  ...
  },
  {
    ...
    description: 'Testing special characters, HTML entities, quotes, and symbols: <script>alert("test")</script> & other dangerous content.',
    expectedTemplate: 'minimal',
    expectedTheme: 'light',
  ...
  },
  {
    ...
    description: 'Testing Unicode characters, emojis, and international text: 你好世界 🌍 Здравствуй мир 🇷🇺 Hello مرحبا 🇸🇦',
    expectedTemplate: 'tech',
    expectedTheme: 'neon',
...
async function extractImageDimensions(imageBuffer: Buffer): Promise<{ width: number; height: number } | null> {
  try {
...
async function runSingleTest(testConfig: TestConfig): Promise<TestResult> {
  ...
  try {
    ...
    const hasCorrectDimensions = dimensions && 
...
export async function runOGTestSuite(): Promise<{
  ...
  for (const testConfig of OG_TEST_SUITE) {
    ...
    if (result.passed) {
      ...
      console.log(`❌ FAIL: ${result.testName.substring(0, 50)}... ${result.error ? `- ${result.error}` : ''}`);
    }
  ...
  console.log(`   Cache Hit Rate: ${summary.cacheStats.hitRate || 'N/A'}`);
...
export async function runHealthCheck(): Promise<boolean> {
  try {
    ...
    const isValid = await validatePNGHeader(imageBuffer) && 
                   imageBuffer.length > 0 && 
                   generationTime < 5000;
