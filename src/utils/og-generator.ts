...
function createModernBlogTemplate(params: OGImageParams, theme: ThemeConfig) {
  const { title, description, publishedTime, tags, readingTime } = params;
  ...
  return {
    ...
    props: {
      style: {
        ...
        backgroundImage: 'radial-gradient(circle at 25% 25%, #1a1a2e 0%, #0a0a0f 50%)',
        padding: DESIGN_SYSTEM.spacing.xl,
            ...
            children: [
              // System Info
              {
                ...
                props: {
                  ...
                  children: `user@piyush-blog:~$ whoami → ${author || 'Piyush Mehta'}`,
                },
              },
  ...
  return {
    ...
    props: {
      style: {
        ...
        flexDirection: 'column',
        backgroundColor: '#0a0a0f',
        backgroundImage: 'linear-gradient(135deg, #0a0a0f 0%, #1a0033 50%, #0a0a0f 100%)',
        fontFamily: 'Inter',
        position: 'relative',
        overflow: 'hidden',
      },
...
function selectOptimalTemplate(params: OGImageParams): string {
  ...
  if (pageType === 'article') {
    if (
      tags?.some((tag) =>
        ['typescript', 'javascript', 'react', 'node', 'api'].includes(tag.toLowerCase())
      )
    ) {
      if (import.meta.env.DEV) {
  ...
  if (pageType === 'services' || pageType === 'about' || pageType === 'contact') {
    if (import.meta.env.DEV) {
...
export async function generateOGImage(params: OGImageParams): Promise<Buffer> {
  try {
    ...
    const templateFunction =
      TEMPLATE_REGISTRY[templateName as keyof typeof TEMPLATE_REGISTRY] ||
