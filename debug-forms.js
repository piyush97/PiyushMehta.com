...
async function debugFormAccessibility() {
  ...
  try {
    ...
    inputs.forEach(input => {
      ...
      console.log(`  - Has Label: ${input.hasLabel} ${input.labelText ? `("${input.labelText}")` : ''}`);
      ...
      const hasAccessibleLabel = input.hasLabel || input.ariaLabel || input.ariaLabelledby || 
                                 (input.placeholder && input.type !== 'password') || input.title;
    ...
    problematicInputs.forEach(input => {
      console.log(`  - Input ${input.index}: ${input.tagName} (${input.type}) - ID: ${input.id || 'none'}`);
    });
