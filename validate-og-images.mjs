...
async function validateBlogArticles() {
  ...
  for (const blogDir of blogDirs) {
    ...
    try {
      ...
      if (result.success) {
        console.log(
          `   ✅ SUCCESS - ${result.status} - ${result.contentType} - ${result.contentLength} bytes`
        );
        successCount++;
      } else {
        console.log(
          `   ❌ FAILED - ${result.status} - ${result.error || "Unknown error"}`
        );
        failureCount++;
      }
...
async function testAllCombinations() {
  ...
  const testDescription =
    "Testing all template and theme combinations for OG image generation";

  for (const template of templates) {
    for (const theme of themes) {
      console.log(`Testing ${template}/${theme}...`);

      const result = await testOGImage(
        testTitle,
        testDescription,
        template,
        theme
...
async function main() {
  ...
  console.log(
    `Successful: ${successCount} (${Math.round(
      (successCount / totalArticles) * 100
    )}%)`
  );
  console.log(
    `Failed: ${failureCount} (${Math.round(
      (failureCount / totalArticles) * 100
    )}%)`
  );
  ...
  if (failureCount > 0) {
    ...
    results
      ...
      .forEach((r) => {
        console.log(
          `   - ${r.article} (${r.template}/${r.theme}): ${r.error || r.status}`
        );
      });
  }
...
async function checkServer() {
  try {
    const response = await fetch(
      `${BASE_URL}/api/og-image?title=test&description=test`
    );
    return response.ok;
  } catch {
    return false;
