...
async function debugRssFeed() {
  ...
  for (const url of urls) {
    try {
      ...
      if (!contentType.includes('xml')) {
        console.log(
          `  ⚠️ Warning: Content-Type is not XML. This could cause RSS readers to reject the feed.`
        );
      }
      ...
      const outputFile = path.join(
        __dirname,
        `rss-debug-${new URL(url).hostname}-${Date.now()}.txt`
      );
      ...
      console.log('  Checks:');
      console.log(
        `    Content starts with XML declaration: ${body.trim().startsWith('<?xml') ? '✅' : '❌'}`
      );
      console.log(`    Contains <rss> tag: ${body.includes('<rss') ? '✅' : '❌'}`);
      console.log(
        `    Contains <html> tag (should be NO): ${body.includes('<html') ? '❌' : '✅'}`
      );
      console.log(
        `    Contains <body> tag (should be NO): ${body.includes('<body') ? '❌' : '✅'}`
      );
      console.log(`    Response size: ${(body.length / 1024).toFixed(2)} KB`);

      if (body.includes('<html') || body.includes('<body')) {
        console.log(
          `  ❗ ERROR: RSS feed contains HTML tags, which means it's being rendered as a webpage instead of XML`
        );
      }

      if (body.includes('<?xml-stylesheet')) {
        console.log(
          `  ⚠️ Warning: RSS feed contains XML stylesheet, which can cause issues with some RSS readers`
        );
      }
    } catch (error) {
  ...
  console.log('-------------------');
  console.log(
    '1. Your RSS feed should be served with Content-Type: application/xml or application/rss+xml'
  );
