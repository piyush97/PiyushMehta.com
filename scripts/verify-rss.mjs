...
async function checkRemoteRss() {
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = client.get(url, (res) => {
      ...
      console.log(
        `${colors.cyan}Cache-Control: ${headers['cache-control'] || 'not set'}${colors.reset}`
      );
      ...
      res.on('end', () => {
        ...
        if (rawData.trim().startsWith('<?xml')) {
          ...
          console.log(
            `${colors.red}WARNING: Content does not start with XML declaration!${colors.reset}`
          );
        ...
        if (rawData.includes('<rss') && rawData.includes('<channel>')) {
          ...
          console.log(
            `${colors.red}WARNING: Content does not appear to be valid RSS!${colors.reset}`
          );
        }
...
async function main() {
  ...
  try {
    ...
    if (SITE_URL !== 'https://piyushmehta.com') {
      ...
      console.log(
        `${colors.cyan}Local RSS file: ${localResult.exists ? `${colors.green}✓ Valid` : `${colors.red}✗ Invalid or missing`}${colors.reset}`
      );
      ...
      console.log(
        `${colors.cyan}Remote RSS feed: ${remoteResult.isXml && remoteResult.hasRssElements ? `${colors.green}✓ Valid` : `${colors.red}✗ Invalid`}${colors.reset}`
      );
      console.log(
        `${colors.cyan}Remote content type: ${remoteResult.contentType.includes('html') ? `${colors.red}✗ HTML (wrong)` : `${colors.green}✓ XML (correct)`}${colors.reset}`
      );
      ...
      if (remoteResult.contentType.includes('html')) {
        console.log(
          `${colors.yellow}- Your RSS feed is being served as HTML, not XML. Check your Content-Type headers in server config.${colors.reset}`
        );
        console.log(
          `${colors.yellow}- Ensure your Astro config and Vercel settings are correctly configured for /rss.xml${colors.reset}`
        );
      }

      if (!remoteResult.isXml || !remoteResult.hasRssElements) {
        console.log(
          `${colors.yellow}- Your remote RSS feed doesn't appear to be valid XML or RSS.${colors.reset}`
        );
      }

      if (localResult.exists && localResult.itemCount === 0) {
        console.log(
          `${colors.yellow}- Your local RSS file exists but doesn't contain any items.${colors.reset}`
        );
      }
    } else {
      console.log(
