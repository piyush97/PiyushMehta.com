...
async function testSeoFiles() {
  ...
  const sitemapPaths = [
    ...
    path.join(projectRoot, '.vercel', 'output', 'static', 'sitemap.xml'),
  ...
  for (const sitemapPath of sitemapPaths) {
    if (fs.existsSync(sitemapPath)) {
      ...
      console.log(
        `${colors.cyan}Valid format: ${isValid ? `${colors.green}✓` : `${colors.red}✗`}${colors.reset}`
      );
      console.log('-'.repeat(80));
    }
  }
  ...
  if (!sitemapFound) {
    console.log(
      `${colors.red}No sitemap.xml file found in any of the expected locations!${colors.reset}`
    );
  }

  // Check RSS files
  const rssPaths = [
    path.join(projectRoot, 'dist', 'rss.xml'),
    path.join(projectRoot, '.vercel', 'output', 'static', 'rss.xml'),
  ...
  for (const rssPath of rssPaths) {
    if (fs.existsSync(rssPath)) {
      ...
      console.log(
        `${colors.cyan}Contains HTML (bad): ${hasHTML ? `${colors.red}✓` : `${colors.green}✗`}${colors.reset}`
      );
      console.log('-'.repeat(80));
    }
  }
  ...
  if (!rssFound) {
    console.log(
      `${colors.red}No rss.xml file found in any of the expected locations!${colors.reset}`
    );
  }
  ...
  if (fs.existsSync(robotsPath)) {
    ...
    console.log(`${colors.cyan}Size: ${(content.length / 1024).toFixed(2)} KB${colors.reset}`);
    console.log(
      `${colors.cyan}Contains Sitemap directive: ${hasSitemap ? `${colors.green}✓` : `${colors.yellow}✗`}${colors.reset}`
    );
    console.log(
      `${colors.cyan}Contains User-agent directive: ${hasUserAgent ? `${colors.green}✓` : `${colors.yellow}✗`}${colors.reset}`
    );
    console.log('-'.repeat(80));
  } else {
  ...
  console.log('-'.repeat(80));
  console.log(
    `${colors.cyan}Sitemap found: ${sitemapFound ? `${colors.green}✓` : `${colors.red}✗`}${colors.reset}`
  );
  console.log(
    `${colors.cyan}RSS feed found: ${rssFound ? `${colors.green}✓` : `${colors.red}✗`}${colors.reset}`
  );
  console.log(
    `${colors.cyan}robots.txt found: ${fs.existsSync(robotsPath) ? `${colors.green}✓` : `${colors.red}✗`}${colors.reset}`
  );
