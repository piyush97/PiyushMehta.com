...
test.describe('OG Image Performance Tests', () => {
    ...
    test('should handle varying content lengths efficiently', async ({ page }) => {
      const contentTests = [
        { title: 'Short', description: 'Brief.' },
        {
          ...
          description: 'This is a medium length description that should test normal use cases.',
        },
        {
          title: 'This is a Very Long Title That Should Test the Dynamic Sizing Feature',
          description:
            'This is a very long description that should test how the system handles longer content and whether it affects generation performance significantly.',
        },
      ];
      ...
      for (const content of contentTests) {
        ...
        console.log(
          `Content length ${content.title.length}/${content.description.length}: ${duration}ms`
        );
      }
    });
  });
  ...
  test.describe('Concurrent Request Performance', () => {
    test('should handle multiple concurrent requests', async ({ browser }) => {
      const contexts = await Promise.all(
        Array.from({ length: performanceBenchmarks.concurrentRequests }, () => browser.newContext())
      );
      ...
      console.log(
        `${performanceBenchmarks.concurrentRequests} concurrent requests: ${totalDuration}ms`
      );
      console.log(
        `Average per request: ${totalDuration / performanceBenchmarks.concurrentRequests}ms`
      );

      // Average per request should be reasonable
      expect(totalDuration / performanceBenchmarks.concurrentRequests).toBeLessThan(
        performanceBenchmarks.maxGenerationTime
      );
  ...
  test.describe('Resource Usage Performance', () => {
    test('should handle memory-intensive operations', async ({ page }) => {
      ...
      const complexContent = {
        ...
        description:
          'This is a complex test case designed to test memory usage with a long description that contains many words and should stress test the image generation process to ensure it handles memory efficiently.',
  ...
  test.describe('Edge Case Performance', () => {
    test('should handle edge cases without significant performance degradation', async ({
      page,
    }) => {
      const edgeCases = [
        {
        ...
        },
        {
          ...
          params: {
            title: 'Max tags test',
            description: 'Testing maximum tags',
            template: 'modern',
            theme: 'dark',
